...
export function ContactClient({ contactInfo }: ContactClientProps) {
  ...
  const infoItems = [
    ...
    { icon: Phone, label: 'WhatsApp', value: contactInfo.phone, href: `https://wa.me/${contactInfo.whatsapp}` },
        ...
        <div className="grid md:grid-cols-[5fr_7fr] gap-8 max-w-5xl mx-auto">
          <ScrollReveal>
            <div className="rounded-2xl border border-white/[0.06] bg-bg-card/60 backdrop-blur-xl p-8">
              ...
              <div className="mt-7">
                <a href={`https://wa.me/${contactInfo.whatsapp}`} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#25D366] px-5 py-3 text-sm font-semibold text-white transition-all hover:brightness-110">
                  <MessageCircle size={18} /> Chat on WhatsApp
                </a>
              </div>
              {(contactInfo.socialLinks.linkedin || contactInfo.socialLinks.instagram || contactInfo.socialLinks.youtube) && (
                <div className="flex gap-3 mt-7 pt-6 border-t border-border-subtle">
                  {contactInfo.socialLinks.linkedin && (
                    <a href={contactInfo.socialLinks.linkedin} aria-label="LinkedIn" className="w-10 h-10 rounded-lg bg-bg-card border border-border-subtle flex items-center justify-center text-text-muted hover:text-text-primary hover:border-border-default transition-all"><Globe size={18} /></a>
                  )}
                  {contactInfo.socialLinks.instagram && (
                    <a href={contactInfo.socialLinks.instagram} aria-label="Instagram" className="w-10 h-10 rounded-lg bg-bg-card border border-border-subtle flex items-center justify-center text-text-muted hover:text-text-primary hover:border-border-default transition-all"><Globe size={18} /></a>
                  )}
                  {contactInfo.socialLinks.youtube && (
                    <a href={contactInfo.socialLinks.youtube} aria-label="YouTube" className="w-10 h-10 rounded-lg bg-bg-card border border-border-subtle flex items-center justify-center text-text-muted hover:text-text-primary hover:border-border-default transition-all"><Globe size={18} /></a>
                  )}
                </div>
              )}
            </div>
